...
from baseline.utils.db_util import read_correspond_answers_from_all_posts
from baseline.utils.str_util import split_into_paragraph
import time
import pandas as pd
import operator
from baseline.sentence_selection.Order.Order_Analysis import get_order_score
from baseline.sentence_selection.Pattern.Pattern_Analysis import get_pattern_score
from baseline.sentence_selection.HTMLTag.HTML_Analysis import get_html_score
from baseline.sentence_selection.Entropy.Entropy_Analysis import get_entropy_score
from baseline.sentence_selection.Entity.Entity_Analysis import get_entity_score, get_entities_from_word_list
from baseline.sentence_selection.Entropy.build_tf_idf_dic import read_voc
from baseline.utils.preprocessing_util import preprocessing_for_ans_sent
from baseline.pathConfig import res_dir
from baseline.utils.csv_utils import write_list_to_csv
from baseline.utils.StopWords import read_EN_stopwords
...
def get_ss(query_word, top_relevant_paragraph_num, top_dq_id_and_sim, stopword):
    ...
    idf_voc = read_voc()
    # question-level
    answers_list = read_correspond_answers_from_all_posts(top_dq_id_and_sim)
    for(q_id, sim) in top_dq_id_and_sim:
        # answer-level
        answers = answers_list[str(q_id)]
    ...
    return [[x[0], x[2]] for x in Normalized_sent_list[:top_relevant_paragraph_num]]
...
if __name__ == "__main__":
    ...
    for query, top_dq_id_and_sim in load_qs_result(rq_res_fpath):
        top_ss = get_ss(query, top_relevant_paragraph_num, top_dq_id_and_sim, StopWords)
