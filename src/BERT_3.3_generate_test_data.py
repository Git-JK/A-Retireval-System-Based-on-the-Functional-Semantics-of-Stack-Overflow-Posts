...
from utils.read.read_utils import read_all_posts_from_trainset, read_all_posts_from_searchset
from utils.text_process.puretext_extract import puretext_extract, preprocess_title
from utils.config import BERT_Q_A_TEST_DATA_STORE_PATH, BERT_Q_QD_TEST_DATA_STORE_PATH
...
def select_test_posts():
    searchset_posts = read_all_posts_from_searchset()
    trainset_posts = read_all_posts_from_trainset()
    searchset_posts = random.sample(searchset_posts, 50000)
    object_posts = []
    for post in searchset_posts:
        if post not in trainset_posts:
            object_posts.append(post)
        if len(object_posts) % 100 == 0:
            print(f"object_posts len {len(object_posts)}")
    return random.sample(object_posts, 20000)

def generate_data():
    post_list = select_test_posts()
    Qd_corpus = []
    a_corpus = []
    q_Qd_pair_list = []
    q_a_pair_list = []
    ...
    for post in post_list:
        Qd_corpus.append(puretext_extract(post['Body']))
        ...
        a_corpus.append(puretext_extract(post_answer))
    tok_Qd_corpus = [doc.split(" ") for doc in Qd_corpus]
    tok_a_corpus = [doc.split(" ") for doc in a_corpus]
    Qd_bm25 = BM25Okapi(tok_Qd_corpus)
    a_bm25 = BM25Okapi(tok_a_corpus)
    for post in post_list:
        title = preprocess_title(post['Title']).split(' ')
        Qd = puretext_extract(post['Body'])
        post_answers = post['Answers']
        post_answers = sorted(post_answers, key= lambda x:x['Score'])
        post_answer = ''
        for answer in post_answers:
        ...
        if post_answer == '':
            post_answer = post_answers[0]['Body']
        a = puretext_extract(post_answer)
        Qd_top10 = Qd_bm25.get_top_n(title, Qd_corpus, n=10)
        neg_sample_num = random.randint(3, 4)
        radom_select_Qd = random.sample(Qd_top10, neg_sample_num)
        if Qd in radom_select_Qd:
            radom_select_Qd.remove(Qd)
        else:
            radom_select_Qd = random.sample(radom_select_Qd, neg_sample_num - 1)
        for neg_qd in radom_select_Qd:
            q_Qd_pair_list.append([" ".join(title), Qd, neg_qd])
        a_top10 = a_bm25.get_top_n(title, a_corpus, n=10)
        neg_sample_num = random.randint(3, 4)
        radom_select_a = random.sample(a_top10, neg_sample_num)
        if a in radom_select_a:
            radom_select_a.remove(a)
        else:
            radom_select_a = random.sample(radom_select_a, neg_sample_num - 1)
        for neg_a in radom_select_a:
            q_a_pair_list.append([" ".join(title), a, neg_a])
        if len(q_Qd_pair_list) >= 50000:
            file_name = f'q_Qd_pair_{Qd_cnt}.pkl'
            file_path = os.path.join(BERT_Q_QD_TEST_DATA_STORE_PATH, file_name)
