...
import torch
from utils.config import BERT_MODEL_SOTRE_PATH, BERT_Q_QD_TEST_TOKEN_STORE_PATH, PRETRAINED_W2V_MODEL_STORE_PATH, BERT_Q_QD_TEST_DATA_STORE_PATH, BERT_Q_A_TEST_DATA_STORE_PATH
...
def evaluate_bert():
    ...
    q_Qd_BertletsTestDataset = BertletsDataset('bert-base-uncased', BERT_Q_QD_TEST_TOKEN_STORE_PATH)
    test_dataloader = DataLoader(q_Qd_BertletsTestDataset, batch_size=1, shuffle=True, num_workers=4)
    test_accuracy = evaluation(test_dataloader, q_Qd_bertlets_model, device)
...
def evalueate_w2v():
    ...
    FN = 0
    for cnt in range(2):
        file_name = f"q_Qd_pair_{cnt}.pkl"
